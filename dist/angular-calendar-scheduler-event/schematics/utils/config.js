...
function addStyleToTarget(project, targetName, stylePath) {
  ...
  return project.targets.set(
    targetName,
    __assign(__assign({}, target), {
      options: __assign(__assign({}, target.options), {
        styles: __spreadArray([stylePath], target.options.styles, true),
      }),
    }),
  );
}
  ...
  return (0, workspace_1.updateWorkspace)(function (workspace) {
    ...
    if (appConfig) {
      ...
      throw new schematics_1.SchematicsException(
        'project configuration could not be found',
      );
    }
  });
}
function isAngularBrowserProject(projectConfig) {
  ...
  return knownBuilders.includes(
    buildConfig === null || buildConfig === void 0
      ? void 0
      : buildConfig.builder,
  );
}
