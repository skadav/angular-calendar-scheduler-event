...
function addModuleImportToModule(
  ...
  if (!moduleSource) {
    throw new schematics_1.SchematicsException(
      'Module not found: '.concat(moduleToImportIn),
    );
  }
  var changes = (0, ast_utils_1.addImportToModule)(
    moduleSource,
    moduleToImportIn,
    importedModuleName,
    importedModulePath,
  ...
  changes
    .filter(function (change) {
      return change instanceof change_1.InsertChange;
    })
    .forEach(function (change) {
      return recorder.insertLeft(change.pos, change.toAdd);
    });
  host.commitUpdate(recorder);
}
function insertAfterImports(source, fileToEdit, toInsert) {
  var allImports = (0, ast_utils_1.findNodes)(
    source,
    ts.SyntaxKind.ImportDeclaration,
  );
  return (0, ast_utils_1.insertAfterLastOccurrence)(
    allImports,
    toInsert,
    fileToEdit,
    0,
...
function isStandaloneComponent(source) {
  ...
  for (var _i = 0, nodes_1 = nodes; _i < nodes_1.length; _i++) {
    ...
    for (var _a = 0, _b = configArg.properties; _a < _b.length; _a++) {
      ...
      if (!ts.isIdentifier(name_1) || name_1.text !== 'standalone') continue;
...
function addImportsToComponent(source, fileToEdit, imports) {
  ...
  if (!componentDecorator) {
    throw new schematics_1.SchematicsException(
      'Could not find @Component decorator',
    );
  }
  ...
  if (importsProperty) {
    ...
    changes.push(
      new change_1.InsertChange(
        fileToEdit,
        insertPos,
        ''.concat(separator, 'imports: [').concat(imports.join(', '), '],'),
      ),
    );
  }
  return changes;
...
function addImportsToNgModule(source, fileToEdit, imports) {
  ...
  if (!ngModuleDecorator) {
    throw new schematics_1.SchematicsException(
      'Could not find @NgModule decorator',
    );
  }
