...
function addPackageJsonDependencies(options) {
  return function (host, context) {
    ...
    var angularCalendarDependency = nodeDependencyFactory(
      'angular-calendar',
      version_names_1.angularCalendarVersion,
    );
    var dateAdapterLibrary = options.dateAdapter;
    var dateAdapterLibraryDependency = nodeDependencyFactory(
      dateAdapterLibrary,
      dateAdapters[dateAdapterLibrary],
    );
    var angularDraggableDroppableDependency = nodeDependencyFactory(
      'angular-draggable-droppable',
      version_names_1.angularDraggableDroppableVersion,
    );
    var angularResizableElementDependency = nodeDependencyFactory(
      'angular-resizable-element',
      version_names_1.angularResizableElementVersion,
    );
    (0, dependencies_1.addPackageJsonDependency)(
      host,
      angularCalendarDependency,
    );
    context.logger.log(
      'info',
      'Added "'
        .concat(angularCalendarDependency.name, '" into ')
        .concat(angularCalendarDependency.type),
    );
    (0, dependencies_1.addPackageJsonDependency)(
      host,
      dateAdapterLibraryDependency,
    );
    context.logger.log(
      'info',
      'Added "'
        .concat(dateAdapterLibraryDependency.name, '" into ')
        .concat(dateAdapterLibraryDependency.type),
    );
    (0, dependencies_1.addPackageJsonDependency)(
      host,
      angularDraggableDroppableDependency,
    );
    context.logger.log(
      'info',
      'Added "'
        .concat(angularDraggableDroppableDependency.name, '" into ')
        .concat(angularDraggableDroppableDependency.type),
    );
    (0, dependencies_1.addPackageJsonDependency)(
      host,
      angularResizableElementDependency,
    );
    context.logger.log(
      'info',
      'Added "'
        .concat(angularResizableElementDependency.name, '" into ')
        .concat(angularResizableElementDependency.type),
    );
    return host;
  };
}
    ...
    return __awaiter(_this, void 0, void 0, function () {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 1:
            workspace = _a.sent();
            project = (0, utils_1.getProjectFromWorkspace)(
              workspace,
              options.projectName,
            );
            ...
            if (options.installToPath) {
              ...
              componentPath = (0, core_1.normalize)(
                project.root + '/' + options.installToPath,
              );
            } else {
            ...
            if (standalone === false) {
              throw new schematics_1.SchematicsException(
                'Component at '.concat(
                  componentPath,
                  ' has standalone: false. Please use a standalone component or select the NgModule option.',
                ),
              );
            }
            updates = getImportInserts(
              componentSource,
              componentPath,
              options.dateAdapter,
            );
            importsChanges = (0, utils_1.addImportsToComponent)(
              componentSource,
              componentPath,
              getCalendarImports(),
            );
            providersChanges = (0, utils_1.addProvidersToComponent)(
              componentSource,
              componentPath,
            ...
            return [
              2 /*return*/,
              (0, schematics_1.mergeWith)((0, schematics_1.source)(host)),
            ];
        }
      });
    });
...
function getAppComponentPath(host, mainPath, project) {
  ...
  for (
    var _i = 0, possiblePaths_1 = possiblePaths;
    _i < possiblePaths_1.length;
    _i++
  ) {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 1:
            ...
            importsChanges = (0, utils_1.addImportsToNgModule)(
              moduleSource,
              appModulePath,
              getCalendarImports(),
            );
            providersChanges = (0, utils_1.addProvidersToNgModule)(
              moduleSource,
              appModulePath,
              [getProviderFunction(options)],
            );
...
function getImportInserts(sourceFile, filePath, dateAdapter) {
  ...
  var updates = [
    ...
    (0, ast_utils_1.insertImport)(
      sourceFile,
      filePath,
      ''.concat(
        getCalendarImports().join(', '),
        ', DateAdapter, provideCalendar',
      ),
      'angular-calendar',
    ),
    (0, ast_utils_1.insertImport)(
      ...
      filePath,
      'adapterFactory',
      'angular-calendar/date-adapters/'.concat(dateAdapter),
    ),
  ];
  if (dateAdapter === 'moment') {
    updates.push(
      (0, utils_1.insertWildcardImport)(
        sourceFile,
        filePath,
        'moment',
        'moment',
